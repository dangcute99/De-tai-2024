...
class TemperatureChartWindow(QMainWindow):
    ...
    def plot_chart(self):
        ...
        for month, temp in zip(self.months, self.temperature_data):
            ...
            self.ax.scatter(month, temp, color=color, s=150)
        ...
        # Tạo đường nối cho các điểm nhiệt độ lớn hơn 20 độ với màu sắc tương ứng
        x_data = []
        y_data = []
        for month, temp in zip(self.months, self.temperature_data):
            if temp > 20:
        ...
        # Vẽ đường nối cho các điểm nhiệt độ lớn hơn 20 độ
        if x_data and y_data:
            self.ax.plot(
                x_data,
                y_data,
                color='red',  # Đặt màu đỏ cho đường nối này
                linestyle='-',  # Đường liền
                linewidth=2,  # Độ dày của đường
                label="Nối điểm > 20°C"
            )

        # Vẽ đường nối cho tất cả các điểm nhiệt độ (màu đen, đường đứt)
        self.ax.plot(
            self.months,
